...
process createSummary{
    ...
    echo "## Identify missing models:"
    echo '+ Models found in '$structuresFolder
    echo '+ Models found at start of the pipeline: ' $foundModelsCount

    echo "## Retrieve structural information"
    echo '+ Search AFDB: ' $retrieve
    if [ "$retrieve" != "false" ] ; then
        echo '+ Models found in AlphaFold Database: '$foundModelsAF
        fi
    echo '+ Use ESM Atlas: ' $model
    if [ "$model" != "false" ] ; then
        echo '+ Models generated by ESMF: '$foundModelsESM
        fi
    echo '+ No. of sequences not matched to a model: ' $structurelessCount
